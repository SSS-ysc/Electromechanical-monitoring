...
// 此处只是定义多缓冲的个数，在程序中还需要另外调用函数开启多缓冲功能
...
//	开双层显示时，受限于SDRAM的速度，使用24位或32位色可能会出现屏幕抖动或花屏的现象，此时请降低LCD驱动时钟
//	开双层显示时，受限于SDRAM的速度，建议使用16位或更低的颜色格式
//
...
//	本例程给LTDC划分的显存为每层 800*480*4*3 字节，预留了32位色开双层显示以及使用三缓冲的空间，
...
// 配置 emWin 每个图层的颜色格式，支持8位色到32位色，用户可根据需要进行配置
//	需要给每个图层合理的划分显存，假设使用16位色，使用三缓冲，则该图层占用 800*480*2*3 字节的SDRAM空间
...
static LCD_PIXELINDEX * _LCD_GetpPalConvTable(const LCD_LOGPALETTE GUI_UNI_PTR * pLogPal, const GUI_BITMAP GUI_UNI_PTR * pBitmap, int LayerIndex)
{
  ...
  /* Load LUT using DMA2D */
	 DMA_LoadLUT((LCD_COLOR *)pLogPal->pPalEntries, pLogPal->NumEntries);
...
static void DMA_Index2ColorBulk(void * pIndex, LCD_COLOR * pColor, uint32_t NumItems, U8 SizeOfIndex, uint32_t PixelFormat)
{
  ...
  DMA_ConvertColor(pIndex, pColor, PixelFormat, LTDC_Pixelformat_ARGB8888, NumItems);
}
...
{
	/* Use DMA2D for the conversion */
	DMA_ConvertColor(pColor, pIndex, LTDC_Pixelformat_ARGB8888, PixelFormat, NumItems);
...
static void LCD_MixColorsBulk(U32 * pFG, U32 * pBG, U32 * pDst, unsigned OffFG, unsigned OffBG, unsigned OffDest, unsigned xSize, unsigned ySize, U8 Intens) {
  ...
  for (y = 0; y < ySize; y++) 
  {
    DMA_MixColorsBulk(pFG, pBG, pDst, Intens, xSize);
    pFG  += xSize + OffFG;
    pBG  += xSize + OffBG;
    pDst += xSize + OffDest;
  }
...
void LCD_X_Config(void) 
{
  ...
  for (i = 0; i < GUI_NUM_LAYERS; i++) {
    ...
    if (GetPixelformat(i) == LTDC_Pixelformat_ARGB8888) {
      LCD_SetDevFunc(i, LCD_DEVFUNC_DRAWBMP_32BPP, (void(*)(void))LCD_DrawBitmap32bpp);     /* Set up drawing routine for 32bpp bitmap using DMA2D. Makes only sense with RGB565 */
    }  
